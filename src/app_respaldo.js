...
function postRenderBindings(){
  ...
  if(currentSection === 'inventario') {
    ...
    // si el hash pide abrir un archivo, initInventory lo tratará (ver pendingOpen)
  }
...
async function downloadMultipleJsonAsDocx(category, filename) {
  const DOCX_CDN = "https://cdnjs.cloudflare.com/ajax/libs/docx/8.3.2/docx.min.js";
  ...
  // tiny helper para cargar docx si es necesario
  async function ensureDocxLoaded(url, timeout = 10000) {
    ...
    if (existing) {
      ...
      return await new Promise((resolve, reject) => {
        let done = false;
        const t = setTimeout(() => { if(!done){ done = true; reject(new Error("timeout cargando docx")); } }, timeout);
        existing.addEventListener('load', () => { if(!done){ done = true; clearTimeout(t); resolve(globalThis.docx); } });
        existing.addEventListener('error', () => { if(!done){ done = true; clearTimeout(t); reject(new Error("error cargando docx")); } });
      });
    }
    ...
    return await new Promise((resolve, reject) => {
      ...
      s.async = true;
      let done = false;
      const timer = setTimeout(() => { if(!done){ done = true; reject(new Error("timeout cargando docx")); } }, timeout);
      s.addEventListener('load', () => { if(!done){ done = true; clearTimeout(timer); resolve(globalThis.docx); } });
      s.addEventListener('error', () => { if(!done){ done = true; clearTimeout(timer); reject(new Error("error cargando docx")); } });
      document.head.appendChild(s);
    });
  }

  try {
    // 1) reunir todas las tablas válidas de la categoría
    const datasets = [];
    for (const lab of labs) {
      const path = `src/excel/${encodeURIComponent(category)}/${encodeURIComponent(lab)}.xlsx`;
      // intenta leer
      const json = await fetchExcelToJson(`src/excel/${category}/${lab}.xlsx`).catch(()=>false);
      if (json && Array.isArray(json)) {
        const cleaned = cleanExcelJson(json);
        if (cleaned.length > 0) {
          datasets.push({ lab, json: cleaned });
        }
      }
    }

    if (!datasets.length) {
      alert(`No se encontraron tablas con datos para la categoría "${category}".`);
      return;
    }

    // 2) intentar usar lib docx
    let docxLib = globalThis.docx;
    if (!docxLib) {
      try { docxLib = await ensureDocxLoaded(DOCX_CDN, 10000); } catch (err) { console.warn("No se pudo cargar docx:", err); docxLib = globalThis.docx; }
    }
    ...
    if (docxLib) {
      ...
      // colores (sin '#')
      const headerFill = "396497";
      const headerColor = "FFFFFF";
      const altRowFill = "EAF4FF";
      const whiteFill = "FFFFFF";

      // Construir secciones: una por cada lab
      const sections = datasets.map((ds, idx) => {
        const keys = Object.keys(ds.json[0]);

        // encabezado fila
        const headerCells = keys.map(key => new docx.TableCell({
          shading: { fill: headerFill },
          children: [ new docx.Paragraph({
            alignment: docx.AlignmentType.CENTER,
            children: [ new docx.TextRun({ text: String(key), bold: true, color: headerColor, size: 24 }) ]
          }) ]
        }));

        const rows = [ new docx.TableRow({ children: headerCells }) ];

        ds.json.forEach((row, rIdx) => {
          const isAlt = (rIdx % 2) === 1;
          const fill = isAlt ? altRowFill : whiteFill;
          const cells = keys.map(k => new docx.TableCell({
            shading: { fill },
            children: [ new docx.Paragraph({ children: [ new docx.TextRun({ text: row[k] != null ? String(row[k]) : "", size: 22 }) ] }) ]
          }));
        ...
        const titleParagraph = new docx.Paragraph({
          children: [ new docx.TextRun({ text: `${category} - ${ds.lab}`, bold: true, size: 26 }) ],
          alignment: docx.AlignmentType.CENTER,
          spacing: { after: 200 }
        });
    ...
    // 3) fallback HTML (.doc) con saltos de página entre labs
    function escapeHtml(s) { return String(s).replace(/[&<>"']/g, m => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m])); }

    let html = `<!doctype html><html><head><meta charset="utf-8"><style>
      body{font-family:Arial, sans-serif;} table{border-collapse:collapse;width:100%;} th{background:#396497;color:#fff;padding:8px;border:1px solid #ccc;text-align:center;}
    ...
    console.error("Error generando DOCX múltiple:", err);
